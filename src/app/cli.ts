...
(async () => {
  ...
  const { cli, envService, migrateService, templateService, randomModule, fsModule, log, connection } = await cliFactory(process.env.ENV_PATH);

  try {
    await cli.register("generate", async () => {
    ...
    log.error({ message: "CLI failed unexpectedly", data: error })
  } finally {
