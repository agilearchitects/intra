...
export type ConstructorParametersType<T extends new (...args: any[]) => any> = T extends new (...args: infer P) => any ? P : never; // tslint:disable-line:max-line-length
...
export class HelperService extends ServiceModule {
    ...
    public promiseToBluebird<T>(promise: Promise<T>): BluebirdPromise<T> {
        return new BluebirdPromise<any>((resolve, reject) =>
            promise.then((result) => resolve(result)).catch((error: any) => reject(error)));
    }
    ...
    public sqlDateFormat(date: Date = new Date()): string {
        ...
        const month: string = padString((date.getMonth() + 1).toString());
        const day = padString(date.getDate().toString());
        const hour = padString(date.getHours().toString());
        const minute = padString(date.getMinutes().toString());
        const second = padString(date.getSeconds().toString());
